...
/** Khớp với giới hạn BE (ImageUploadService): 5MB, ảnh raster phổ biến. */
...
interface Props {
  ...
  /** Ảnh hiện tại của tài khoản (khi sửa) — hiện làm nền khi chưa chọn ảnh mới. */
...
export function AvatarDropzone({ file, onChange, currentUrl }: Props) {
  ...
  // Tạo/thu hồi object URL trong handler, không dùng effect (tránh render thừa).
  ...
  // Dọn URL cuối cùng khi đóng form.
  useEffect(() => () => {
    if (previewUrlRef.current) URL.revokeObjectURL(previewUrlRef.current);
  }, []);
