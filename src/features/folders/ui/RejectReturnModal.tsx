...
export function RejectReturnModal({ fileName, busy, onClose, onSubmit }: RejectReturnModalProps) {
  const [rejectReason, setRejectReason] = useState('');
  const canSubmit = rejectReason.trim().length > 0 && !busy;
      ...
      <div className="relative z-10 w-full max-w-md animate-scale-in rounded-(--radius-card-lg) bg-card shadow-modal">
        <div className="flex items-center justify-between border-b border-card-border px-6 py-4">
          <div className="min-w-0">
            <h2 className="font-heading text-lg font-bold text-text">{t('returnRequests.rejectModal.title')}</h2>
        ...
        <div className="space-y-4 px-6 py-5">
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-bold uppercase tracking-wider text-text-muted">{t('returnRequests.rejectModal.reasonLabel')}</span>
            <textarea
              autoFocus
              rows={4}
              value={rejectReason}
              ...
              disabled={busy}
              placeholder={t('returnRequests.rejectModal.reasonPlaceholder')}
        ...
        <div className="flex justify-end gap-3 border-t border-card-border px-6 py-4">
          <button type="button" onClick={onClose} disabled={busy} className="rounded-xl px-4 py-2 text-sm font-semibold text-text-secondary transition-colors hover:bg-content-bg disabled:opacity-40">
            {t('returnRequests.rejectModal.cancel')}
          </button>
          <button
            type="button"
            disabled={!canSubmit}
          ...
          >
            {busy ? t('common.loading') : t('returnRequests.rejectModal.submit')}
          </button>
        </div>
      </div>
    </div>
