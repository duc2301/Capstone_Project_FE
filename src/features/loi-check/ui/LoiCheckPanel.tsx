...
// Tra bảng tường minh thay vì ghép chuỗi rồi ép kiểu, để TypeScript còn canh được key.
...
// BE đặt Unknown khi không cấu kiện nào vào được vòng chấm: coveragePercent = 0 và
// missing rỗng vì KHÔNG ĐO ĐƯỢC, không phải vì đạt.
function isNotEvaluated(result: LoiCheckResult): boolean {
  return result.verdict === Verdict.Unknown || result.verdict === Verdict.None;
}

// Cùng "không đo được" nhưng lý do khác nhau -> nói đúng lý do người dùng mới biết sửa ở đâu.
...
// Panel hẹp nên chỉ đưa trạng thái từng lớp; chi tiết vi phạm để dành cho trang báo cáo.
...
// Tách khỏi "chưa khai loại": không phải lỗi model, gộp lại người dùng đi tìm chỗ sửa vô ích.
...
// Mỗi đơn vị đặt tên một kiểu: hệ thống đoán, người dùng xác nhận một lần, lần sau tự nhận.
...
export function LoiCheckPanel({ fileItemId, projectId }: { fileItemId: string; projectId?: string }) {
  ...
  // Trang báo cáo neo theo dự án, không có projectId thì chỉ xem được bản tóm tắt.
