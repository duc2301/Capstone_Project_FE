...
export function useLoiMatrix(projectId: string | null, componentId: string | null) {
  const [saving, setSaving] = useState(false);

  const { data: matrix, loading, error, setData, reload } = useAsyncData<LoiMatrix | null>(
    `${projectId ?? 'none'}|${componentId ?? 'none'}`,
    async () => {
      if (!projectId || !componentId) return null;
      const { data } = await loiRuleApi.getMatrix(projectId, componentId);
      return unwrap(data);
    },
    {
      fallback: null,
  ...
  const runMutation = useCallback(async (action: () => Promise<LoiMatrix>) => {
    if (!projectId || !componentId) throw new Error(t('loiRule.error.noComponent'));
    setSaving(true);
    try {
  ...
  }, [projectId, componentId, setData]);

  const saveVariant = useCallback(
    (variant: string | null, rows: LoiMatrixRow[]) =>
      runMutation(async () => {
        const { data } = await loiRuleApi.saveMatrix(projectId!, componentId!, { variant, rows });
        return unwrap(data);
      }),
    [projectId, componentId, runMutation],
  );

  const renameVariant = useCallback(
    (currentVariant: string | null, newVariant: string | null) =>
      runMutation(async () => {
        const { data } = await loiRuleApi.renameVariant(projectId!, componentId!, {
          currentVariant,
          newVariant,
        });
        return unwrap(data);
  ...
  const deleteVariant = useCallback(
    (variant: string | null) =>
      runMutation(async () => {
        const { data } = await loiRuleApi.deleteVariant(projectId!, componentId!, variant);
        return unwrap(data);
      }),
    [projectId, componentId, runMutation],
  );
