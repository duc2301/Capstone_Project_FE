  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <h2 className="heading-entity">{t('markup.model.title')}</h2>
      ...
      {selectedId && !drawing && (
        <div className="flex items-center justify-between gap-2 rounded-xl border border-primary/30 bg-primary/5 px-3 py-2">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded-[var(--radius-button)] bg-text px-2.5 py-1 text-xs font-semibold text-white transition-opacity hover:opacity-90"
          >
      ...
      {readOnly ? null : drawing ? (
        <div className="space-y-2 rounded-xl border border-card-border bg-content-bg/50 p-3">
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={cancelDraw} disabled={saving} className="rounded-[var(--radius-button)] px-2.5 py-1.5 text-xs font-semibold text-text-muted hover:bg-content-bg disabled:opacity-50">
              ...
              <li key={note.id} className={`overflow-hidden rounded-xl border transition-colors ${selected ? 'border-primary ring-1 ring-primary/40' : 'border-card-border'}`}>
                ...
                >
                  {note.thumbnailDataUrl ? (
                    <img src={note.thumbnailDataUrl} alt={t('markup.model.scene')} className="h-32 w-full bg-viewer-shell object-cover" />
                  ) : (
                ...
                <div className="p-3">
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <span className="text-xs font-semibold text-text-secondary">{note.authorName ?? '-'}</span>
