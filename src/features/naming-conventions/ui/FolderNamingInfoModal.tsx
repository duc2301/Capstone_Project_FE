...
interface FolderNamingInfoModalProps {
  ...
  /** Admin hoặc Leader — false thì chỉ xem, 2 nút hành động bị khóa. */
  canManage: boolean;
  onClose: () => void;
  /** Gọi sau khi kế thừa thành công — cha refetch/toast. */
  onInherited: () => void;
  /** Gọi sau khi tạo bản quy tắc riêng (clone) thành công. */
  onCustomized: () => void;
}

/* Tóm tắt 1 convention của folder: badge + delimiter + danh sách trường. */
...
/* Xem quy tắc đặt tên của 1 thư mục + nút "Kế thừa từ thư mục cha".
 * ("Tùy chỉnh riêng" chờ BE clone endpoint — hiện disabled.) */
export function FolderNamingInfoModal({ folder, canManage, onClose, onInherited, onCustomized }: FolderNamingInfoModalProps) {
  ...
  // Tùy chỉnh riêng = clone convention hiện tại (ưu tiên của chính folder, không có thì của cha)
  // thành bản riêng gán cho folder này — leader thêm/bớt field không ảnh hưởng bản gốc.
  const sourceConventionId = current?.hasNamingConvention
    ? current.namingConventionId
    : parent?.hasNamingConvention
      ? parent.namingConventionId
      : null;

  const handleCustomize = async () => {
    if (!sourceConventionId) return;
    setBusy(true);
    setError(null);
    try {
      await namingConventionApi.cloneForFolder(sourceConventionId, folder.id);
      onCustomized();
    } catch (err) {
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap justify-end gap-3 border-t border-card-border pt-4">
            <button
              ...
              disabled={busy || !sourceConventionId || !canManage}
