    ...
    <>
      ...
      <aside
        className={`
          ...
          ${isCompact ? 'w-[76px]' : 'w-[260px]'}
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}
        `}
      >
        {/* ── Brand (cố định) ─────────────────────── */}
        <div className={`flex shrink-0 items-center bg-[#D7DBCA] px-4 py-5 ${isCompact ? 'justify-center' : 'justify-between px-6'}`}>
          {!isCompact && (
          ...
          <button
            ...
            className="hidden h-8 w-8 items-center justify-center rounded-lg text-primary/70 transition-colors hover:bg-black/5 hover:text-primary lg:flex"
          >
        ...
        <nav className="admin-scrollbar min-h-0 flex-1 overflow-y-auto px-3 pt-4">
          {visibleSections.map((section, idx) => (
            <div key={section.titleKey ?? `top-${idx}`} className={idx === 0 ? '' : 'mt-5'}>
              {section.titleKey && !isCompact && (
                <p className="px-6 pb-1.5 text-[10px] font-bold uppercase tracking-wider text-sidebar-text/60">
                  {t(section.titleKey)}
                </p>
              )}
        ...
        <div className="shrink-0 border-t border-white/10 px-3 py-3">
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl py-2.5 text-sm font-medium text-sidebar-text transition-all duration-200 hover:bg-white/[0.06] hover:text-white disabled:opacity-50 ${isCompact ? 'justify-center px-0' : 'px-4'}`}
          >
            <IconLogout />
