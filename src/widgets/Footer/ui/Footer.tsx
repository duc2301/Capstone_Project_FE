...
export const Footer = () => {
  return (
    <footer className="rounded-t-[32px] bg-footer-dark">
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-x-8 gap-y-12 px-12 py-12 md:grid-cols-4">
        <div className="col-span-2 flex flex-col gap-4 md:col-span-1">
          <span className="font-display text-2xl font-bold text-accent-amber-bright">
            {t('brand.name')}
          </span>
          <p className="text-xs font-medium leading-4 tracking-[0.6px] text-white">
            {t('footer.copyright')}
          </p>
        </div>
        ...
        {COLUMNS.map((column) => (
          <nav key={column.titleKey} className="flex flex-col gap-3">
            <h4 className="heading-label text-white">
              {t(column.titleKey)}
            </h4>
            {column.links.map((link) => (
              <a
                key={link}
                href="#"
                className="text-sm font-semibold tracking-[0.14px] text-white underline underline-offset-2 transition-opacity hover:opacity-80"
              >
                {t(link)}
              </a>
            ))}
